...
def getColFeatures(dataName, dataInfo):
    ...
    for key, value in colFeatures.items():
        if (
            ...
            colFeatures[key] = [
                str(k)[2:4] + "/" + twoDigits(months[i]) + "/" + twoDigits(j)
                for k in years
...
def getEncoding2Type(dataInfo, dataName):
    ...
    columns = [
        ...
        if column in dataInfo["nominal"]
        or column in dataInfo["temporal"]
        or column in dataInfo["quantitative"]
    ]

    # LabelEncoder
    labelencoder = LabelEncoder()
    # ['NO2(ppb)O3(ppb)', 'NO2(ppb)PM2.5(ug/m3)', ...,'yearstation', 'yearweekDay']
    all_types = [col1 + col2 for col1 in columns for col2 in columns if col1 != col2]
...
def enumerateVis(dataInfo, dataName="AQ"):  # based on DeepEye P.8  # y不能是nominal
    ...
    if q_cols and n_cols:
        ...
        enumerateVizs.extend(
            [
                Vis(x=n_col, x_type="n", y=q_col, y_type="q", y_aggre=y_aggre)
                for n_col in n_cols
                for q_col in q_cols
            ]
        )
    if n_cols and (
        dataName == "police" or dataName == "Transaction" or dataName == "Netflix"
    ):
        enumerateVizs.extend(
            [
                Vis(
                    ...
                    y_type="n",
                    y_aggre=y_aggre,
                    globalAggre=gloAggre,
                )
                for n_col_x in n_cols
                for y_aggre in aggres
                for gloAggre in globalAggre
                if y_aggre == "cnt"
            ]
        )
    ...
    if t_cols and q_cols:
        ...
        enumerateVizs.extend(
            [
                Vis(x=t_col, x_type="t", y=q_col, y_type="q", y_aggre=y_aggre)
                for t_col in t_cols
                for q_col in q_cols
            ]
        )
...
def getCopyVis(pre_vis, autoSetVisInfo=True):
    vis = Vis(
        mark=pre_vis.mark,
        x=pre_vis.x,
        x_type=pre_vis.x_type,
        y=pre_vis.y,
        y_type=pre_vis.y_type,
        y_aggre=pre_vis.y_aggre,
        z=pre_vis.z,
        z_type=pre_vis.z_type,
        globalAggre=pre_vis.globalAggre,
        Filter=pre_vis.filter.copy(),
        autoSetVisInfo=autoSetVisInfo,
    )
    return vis
    ...
    def getPoints(self):
        ...
        if not self.pre_vis:  # 沒有前人的
            for id, point in data.items():
                if (
                    self.matchFilter(point, self.filter)
                    and self.y in point
                    and point[self.y] != "-"
                ):
            ...
            points = {
                ...
                if len(value) > 0 and key != "nan" and key != ""
            }
            total = len(data)
            ...
            for key, old_points in self.pre_vis.points.items():
                try:
                    points[key] = {
                        pointID: value
                        for pointID, value in old_points.items()
    ...
    def getSubgroup(self, y_aggre=""):
        ...
        for key in list(colFeatures[self.x]):
            if key in self.points:
                ...
                subgroup[key] = self.aggregate(y_aggre, self.points[key].values())
