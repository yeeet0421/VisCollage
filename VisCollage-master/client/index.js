...
function updateSeqViewByExloreView(new_tree_structure=false){
    ...
    $.ajax({
        ...
        ////////////// Freely drag charts //////////////
        if(chart_id_list.length > 0){
            for(let i=0;i<chart_id_list.length;i++){
                if(chart_id_list[i]!="root"){
                    //let chart_div = chart_id_list[i].substring(1) + '-clone'
                    //dragElement(document.getElementById(chart_div))
                    //$( chart_div ).draggable();
                }
            }
        }
...
function addElement(par_element,canvas_id,chart_data,chart_id=""){ 
    ...
    ///////// revised 2020.01 ////////////
    //title
    var title = titleDiv(chart_data) //[title substring(0,40), title full name]
    ...
    if(curr_dataset=="Covid"){
        if(Object.getOwnPropertyNames(chart_data.filters).length!=0){
            if(chart_data.filters["Country/Region"].includes("Italy")){
                var idx = chart_data.labels.indexOf("20/05/14")
                chart_data.datas[0].data[idx] = 899
            }
            if(chart_data.filters["Country/Region"].includes("India")){
                var idx = chart_data.labels.indexOf("20/06/10")
                chart_data.datas[0].data[idx] = 9985

                var idx = chart_data.labels.indexOf("20/06/11")
                chart_data.datas[0].data[idx] = 11586

                var idx = chart_data.labels.indexOf("20/06/12")
                chart_data.datas[0].data[idx] = 11567

                var idx = chart_data.labels.indexOf("20/06/15")
                chart_data.datas[0].data[idx] = 11589

                var idx = chart_data.labels.indexOf("20/07/28")
                chart_data.datas[0].data[idx] = 49981

                var idx = chart_data.labels.indexOf("20/07/29")
                chart_data.datas[0].data[idx] = 49982

                var idx = chart_data.labels.indexOf("20/09/14")
                chart_data.datas[0].data[idx] = 73930

                var idx = chart_data.labels.indexOf("20/09/15")
...
function drawConnectors(drill_tree_structure,comparison_tree_structure){
   ...
   Object.keys(node_data).forEach(function(item,i){
    ...
    var node = svg
        ...
        .attr("id", function(d){return d.name+"-avatar";})
    ...
    node.append("text")
        ...
        .filter(function(d) { return d.x < svg.width / 2; })
    ...
    var comparison_link_data = Object.keys(comparison_tree_structure).map(function(parent_id,i){
        ...
        //if(switch_connector == false){ // drill = right, comparison = bottom
        ...
        //}else{ // drill = bottom, comparison = right
        //    var parent_coordinate = {
        //        y:(parent_block.top + parent_block.width/2) - svg_block.top , 
        //        x:parent_block.right - svg_block.left
        //    }
        //    var child_coordinate = {
        //        y:(child_block.top + child_block.width/2) - svg_block.top, 
        //        x:child_block.left - svg_block.left
        //    }
        //}
    ...
    for(let i=0;i<values.length;i++){
        ...
        for(let j=0;j<list.length;j++){
            if(list[j] && parseInt(list[j].chart_index) == chart_index){
                ...
                chart_datas[curr_dataset][main_chart_id].rec[main_insight_key][keys[i]][j].label =label
                chart_datas[curr_dataset][main_chart_id].rec[main_insight_key][keys[i]][j].edgeValue =list[j].edgeValue
...
function onCloseChart(){
    ...
    */
    ...
    chart_datas[curr_dataset][chart_id.substring(1)].is_selected = false;
...
function clean(){
    ...
    document.getElementById('gen_comics').style.visibility = "hidden"
    document.getElementById('thumbnail_toggle').style.visibility = "hidden"
...
Coloris({
    ...
    swatches:["#003065","#0068b2","#0095C7","#83c7e0","#cbe5f1","#652F00","#006508","#510065"]
  });
...
$(document).ready(function(){   
    ...
    $('#add_a_chart_confirm_btn').click(function(){
        ...
        document.getElementById('gen_comics').style.visibility = "visible"
        document.getElementById('thumbnail_toggle').style.visibility = "visible"
    ...
    ///////////////// Add to gen poster ////////////////////
    $('#gen_poster').click(function(e){
        if(Object.keys(tree_structures[curr_dataset][curr_sheet_num-1]).length > 2){
            document.getElementsByClassName('mask')[0].style.display="block";
            drawCollage(tree_structures[curr_dataset][curr_sheet_num-1],chart_datas[curr_dataset]);
            //screenshot();
        }
        else{
            // not enough to render
            alert('It\'s not enough to render the collage!');
        }
    });

    $('#gen_comics').click(function(){
        document.getElementsByClassName('mask')[1].style.display="block";
        drawComics(tree_structures[curr_dataset][curr_sheet_num-1],chart_datas[curr_dataset]);
        //screenshot();
    });

    ///////////////// Add connector toggle ////////////////////
    var checkbox = document.querySelector('input[name="thumbnail_checkbox"]');
    checkbox.addEventListener('change', function () {
        if (checkbox.checked) {
            // do this
            document.getElementById('explore_view_'+curr_sheet_num).style.display="none";
            //switch_connector = true;
            //explore_views[curr_sheet_num-1].tree.rootOrientation = "NORTH"
            //explore_views[curr_sheet_num-1].tree.nodeAlign = "CENTER";
            //updateSeqViewByExloreView(false);
            
        } else {
            // do that
            document.getElementById('explore_view_'+curr_sheet_num).style.display="block";
            //switch_connector = false;
            //explore_views[curr_sheet_num-1].tree.rootOrientation = "WEST";
            //explore_views[curr_sheet_num-1].tree.nodeAlign = "BOTTOM";
            //updateSeqViewByExloreView(false);
        }
    });

    ///////////////// Poster Page Tags ////////////////////
    $('#poster_banner').on('click', '.button', function() {
        //hide old sheet container
        var poster_container = document.getElementById('poster_'+curr_poster_num);
        if (poster_container){
            poster_container.style.display = "none";
            $("#P"+curr_poster_num).removeClass('active');
            // remove leaderline
        ...
        curr_poster_num = $(this).attr('id').substring(1);
        poster_container = document.getElementById('poster_'+curr_poster_num);
        var leave_div = document.getElementById('leave_chart_div_'+curr_poster_num);
        if(poster_container){
            poster_container.style.display = "block";
            for(var l=0; l< leave_div.children.length; l++){
                drawForceLine(curr_poster_num, leave_div.children[l], annoBox_infos[parseInt(curr_poster_num)].lineStartDiv[l]);
            }
            //$("#P"+curr_poster_num).addClass('active');
        }
    });


    ///////////////// Editor Tags ////////////////////
    $('#layout1').on('click', function() { curr_poster_layout[curr_poster_num] = 1; EditPosterLayout(1)} );
    $('#layout2').on('click', function() { curr_poster_layout[curr_poster_num] = 2; EditPosterLayout(2)} );
    $('#layout3').on('click', function() { curr_poster_layout[curr_poster_num] = 3; EditPosterLayout(3)} );


    ///////////////// Comic Page Tags ////////////////////
    $('#comics_banner').on('click', '.button', function() {
        var storypiece_container = document.getElementById('piece'+curr_comic_num);
        if (storypiece_container){
            storypiece_container.style.display = "none";
            $("#C"+curr_comic_num).removeClass('active');
        }

        curr_comic_num = $(this).attr('id').substring(1);
        storypiece_container = document.getElementById('piece'+curr_comic_num);
        if (storypiece_container){
            storypiece_container.style.display = "block";
            //$("#C"+curr_comic_num).addClass('active');
        }
    });

    ///////////////// Right Click ////////////////////
    $(".annoBox").mousedown(function(ev){
        if(ev.which == 3)
        {
...
function ClosePoster(){
    ...
    while (poster_view.lastElementChild.id !== "poster_banner") {
        poster_view.lastChild.remove();
    }
    // remove poster tag
    var poster_banner = document.getElementById('poster_banner');
    while (poster_banner.lastElementChild.id !== "") {
        poster_banner.lastChild.remove();
    }

    // hidden mask
    mask = document.getElementsByClassName('mask');
    try{
        mask[0].getElementsByClassName("tw-map").remove();
    }catch(exception){}
...
function CloseComics(){
    ...
    while (comics_view.lastElementChild.id !== "comics_banner") {
        comics_view.lastChild.remove();
    }
    // remove poster tag
    var comics_banner = document.getElementById('comics_banner');
    while (comics_banner.lastElementChild.id !== "") {
        comics_banner.lastChild.remove();
    }
    // hidden mask
    mask = document.getElementsByClassName('mask');
...
function screenshot(poster_num){
