...
def getColFeatures(dataName, dataInfo):
    ...
    for point in data.values():
        for key, value in point.items():
            # city,station,year,month,day,weekday
            if key in dataInfo['nominal'] or key in dataInfo['temporal']:
    ...
    for key, value in colFeatures.items():
        if key == 'year' or key == 'month' or key == 'day' or key == 'invoice_month' or key == 'invoice_day' \
            ...
            colFeatures[key] = ['Mon', 'Tue',
                                'Wed', 'Thu', 'Fri', 'Sat', 'Sun']
        elif key == 'Date':  # handle the covid dataset
            years, months, days = getDateInfo(data)
            #print(years, months, days)
            colFeatures[key] = [str(k)[2:4] + "/" + twoDigits(months[i]) + "/" + twoDigits(
                j) for k in years for i in range(len(months)) for j in range(1, days[i-1]+1)]
...
def getEncoding2Type(dataInfo, dataName):
    ...
    # ['NO2(ppb)', 'O3(ppb)', 'PM2.5(ug/m3)', 'SO2(ppb)', 'city', 'day', 'month', 'station', 'weekDay', 'year']
    columns = list(dataInfo['colFeatures'].keys())
    columns = [column for column in columns if column in dataInfo['nominal']
               or column in dataInfo['temporal'] or column in dataInfo['quantitative']]

    # LabelEncoder
    labelencoder = LabelEncoder()
    # ['NO2(ppb)O3(ppb)', 'NO2(ppb)PM2.5(ug/m3)', ...,'yearstation', 'yearweekDay']
    all_types = [
        col1+col2 for col1 in columns for col2 in columns if col1 != col2]
    ...
    for col1 in columns:
        for col2 in columns:
            if col1 != col2:
                # index_0: result['NO2(ppb)']['O3(ppb)']  = [1.0, 0.0, 0.0, ..., 0.0]
                result[col1][col2] = list(ohe_X[index])
                index += 1
            else:
...
def enumerateVis(dataInfo, dataName='AQ'):  # based on DeepEye P.8  # y不能是nominal
    ...
    if q_cols and n_cols:
        ...
        enumerateVizs.extend([Vis(x=n_col, x_type='n', y=q_col, y_type='q',
                             y_aggre=y_aggre) for n_col in n_cols for q_col in q_cols])
    if n_cols and (dataName == 'police' or dataName == 'Transaction' or dataName == 'Netflix'):
        enumerateVizs.extend([Vis(x=n_col_x, x_type='n', y=n_col_x, y_type='n', y_aggre=y_aggre, globalAggre=gloAggre)
                             for n_col_x in n_cols for y_aggre in aggres for gloAggre in globalAggre if y_aggre == 'cnt'])
    ...
    if t_cols and q_cols:
        y_aggre = "avg" if dataName == "AQ" else "sum"
        enumerateVizs.extend([Vis(x=t_col, x_type='t', y=q_col, y_type='q',
                             y_aggre=y_aggre) for t_col in t_cols for q_col in q_cols])
...
def getCopyVis(pre_vis, autoSetVisInfo=True):
    vis = Vis(mark=pre_vis.mark, x=pre_vis.x, x_type=pre_vis.x_type, y=pre_vis.y, y_type=pre_vis.y_type, y_aggre=pre_vis.y_aggre,
              z=pre_vis.z, z_type=pre_vis.z_type, globalAggre=pre_vis.globalAggre, Filter=pre_vis.filter.copy(), autoSetVisInfo=autoSetVisInfo)
    return vis
...
class Vis():
    ...
    def getPoints(self):
        ...
        #    }
        # }
        points = defaultdict(lambda: defaultdict(lambda: defaultdict(
            lambda: 0))) if self.z else defaultdict(lambda: defaultdict())
        ...
        if not self.pre_vis:  # 沒有前人的
            for id, point in data.items():
                if self.matchFilter(point, self.filter) and self.y in point and point[self.y] != '-':
                    ...
                    if self.z:
                        points[point[self.z]][point[self.x]
                                              ][id] = point[self.y]
                    else:
                        # TODO 變成tuple
                        points[point[self.x]][id] = point[self.y]

            points = {key: value for key, value in points.items() if len(
                value) > 0 and key != "nan" and key != ""}
            total = len(data)
            ...
            for key, old_points in self.pre_vis.points.items():
                try:
                    points[key] = {pointID: value for pointID, value in old_points.items(
                    ) if self.matchFilter(data[pointID], self.filter)}
                except:
                    print("Key Error: ")
            points = {key: value for key,
                      value in points.items() if len(value) > 0}
            total = sum([len(value)
                        for key, value in self.pre_vis.points.items()])

        return points

    def getSubgroup(self, y_aggre=""):
        subgroup = defaultdict(lambda: defaultdict(
            lambda: 0)) if self.z else defaultdict(lambda: 0)
        newSubgroup = defaultdict(lambda: defaultdict(
            lambda: 0)) if self.z else defaultdict(lambda: 0)
        ...
        for key in list(colFeatures[self.x]):
            if key in self.points:
                ...
                # key: 2018  #subgroup[key]: 20.0
                subgroup[key] = self.aggregate(
                    y_aggre, self.points[key].values())
        ...
        if self.globalAggre == 'per':
            ...
            if self.x in dataInfos[curr_data]['nominal']:
                subgroup = dict(sorted(newSubgroup.items(),
    ...
    def aggregate(self, aggre, points):
        ...
        if aggre != 'cnt':
            points = list(map(lambda value: float(value.replace(',', '')) if isinstance(
                value, str) else float(value), points))  # handle 1,134.58
...
def setDataInfos():
    ...
    for dataName, dataInfo in dataInfos.items():
        ...
        dataInfo['data'] = {point[dataInfo['ID_col']]
            : point for point in data_point}
...
def optimal_scale_placement(canvas_info, weight):
    ...
    origin_info = {'shape_regions': [], 'alpha_regions': [],
                   'insights': canvas_info['insights']}

    for i in range(0, len(canvas_info['regions'])):
        origin_info['shape_regions'].append(
            canvas_info['regions'][i]['points'])
...
def AABBArea(A, B, pt, ln):
    length = 0
    if A[pt] <= B[pt] and A[pt] + A[ln] <= B[pt] + B[ln]:
        length = A[pt] + A[ln] - B[pt]
    elif A[pt] <= B[pt] and A[pt] + A[ln] >= B[pt] + B[ln]:
        length = B[ln]
    elif A[pt] >= B[pt] and A[pt] + A[ln] >= B[pt] + B[ln]:
        length = B[pt] + B[ln] - A[pt]
    elif A[pt] >= B[pt] and A[pt] + A[ln] <= B[pt] + B[ln]:
        length = A[ln]
    return length
...
def optimization(info, weight):
    ...
    for t in range(0, len(TARGET_C)):
        ...
        bounds = [[0, info['width']-BBOX_SIZE[0]],
                  [0, info['height']-BBOX_SIZE[1]]]
        # perform the simulated annealing search
        result = dual_annealing(objective, bounds, args=(
            polys, TARGET_C[t], BBOX_SIZE))
        # print("=====================", result, "+++++++++++++++++++++++++")
        ...
        solution_c = (solution[0] + BBOX_SIZE[0]/2,
                      solution[1] + BBOX_SIZE[1]/2)

        MIN_C.append(solution_c)
        MIN_SCORE.append(evaluation)
        polys.append({'points': [solution[0], solution[1],
                     BBOX_SIZE[0], BBOX_SIZE[1]], 'alpha': weight})
