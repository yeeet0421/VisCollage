const raw = `Alabama,Montgomery,32.377716,-86.300568
...
export default raw.split('\n').map((line) => {
  const [name, description, latitude, longitude] = line.split(',');
  return { name, description, latitude: Number.parseFloat(latitude), longitude: Number.parseFloat(longitude) };
});
