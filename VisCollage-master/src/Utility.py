...
def edgeValue(rec_cand, regressionModel=None):
    ...
    features = list(map(lambda vis: list(
        chain(*list(vis.features.values()))), rec_cand))
    #
    for f1 in features:
        if np.isinf(f1).any():
            # print(f1)
        ...
        if np.isnan(f1).any():
            ...
            for j in range(len(f1)):
                if np.isnan(f1[j]):
                    f1[j] = 0
    print("IsInf: ", np.isinf(features).any())
    print("IsFinite: ", np.isfinite(features).all())
    print("IsNan: ", np.isnan(features).any())
    #
    if regressionModel == None:
        ...
        if len(list(regressionModel.coef_)) != len(features[0]):
            ...
            edgeValues = [sum(map(operator.mul, weights, feature))
...
def setVisFeature(curr_vis, isRoot=False):
    if isRoot:
        ...
        curr_vis.features['X_encodingType'] = encodingType(
            curr_vis, curr_vis.par_vis, 'x')
        curr_vis.features['Y_encodingType'] = encodingType(
            curr_vis, curr_vis.par_vis, 'y')
    else:
        curr_vis.features['IS'] = [insightSig(curr_vis)]
        curr_vis.features['IG'] = [infoGain(curr_vis, curr_vis.pre_vis)]
        curr_vis.features['expandType'] = expandType(
...
def infoGain(curr_vis, pre_vis):
    ...
    try:
        ...
        # do normalize to the distribution # bound (0,1)
        dist = jensenshannon(curr_values, pre_values, 2)
    except:
        return 0.0
...
def contentImportance(curr_vis, pre_vis):  # correlation #改成3col 也可以算的
    ...
    curr_values = list(curr_subgroup.values())
    pre_values = [pre_subgroup[key]
                  for key in curr_subgroup.keys() if key in pre_subgroup]

    if len(curr_values) == len(pre_values):
        G = sum(curr_values)
        curr_pd = list(map(lambda x: x/G if G != 0 else 0,
                       curr_values))  # normalize to 0-1
...
def trainRegression(train_X, train_y, model, init_model_option):
    X = np.reshape(train_X, (len(train_X), len(train_X[0])))
    y = np.array(train_y)
    print("init_model_option: ", init_model_option)

    if init_model_option == "scratch":
        print("From scratch init model , len (y) = ", len(y))
        regressionModel = linear_model.SGDRegressor(
            max_iter=1000, tol=1e-3, warm_start=True).fit(X, y)
        init_model_option = ""
        ...
        for f1 in X:
            ...
            if np.isnan(f1).any():
                ...
                for j in range(len(f1)):
                    if np.isnan(f1[j]):
                        f1[j] = 0
        print("_IsInf: ", np.isinf(X).any())
        print("_IsFinite: ", np.isfinite(X).all())
        print("_IsNan: ", np.isnan(X).any())
        #
...
def getDecayingLabel(train_y):
    ...
    for i, labels in enumerate(train_y):
        ...
        if diff < 0:
            result.extend(
                list(map(lambda label: label * math.pow(decay_rate, abs(diff)), labels)))
        else:
