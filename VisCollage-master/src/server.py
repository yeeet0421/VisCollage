...
def getChartData(vis, index=None, rank=0):
    ...
    if vis.x in tool.dataInfos[tool.curr_data]['nominal']:
        sorted_group = dict(sorted(vis.subgroup.items(),
    ...
    values = list(map(lambda value: round(value, 2),
                  list(sorted_group.values())))
    keys = list(sorted_group.keys())
    label = 'Overall' if len(vis.filter) == 0 else ', '.join(
        [key+':'+','.join(list(map(lambda value:str(value) if value != "" else "none", value))) for key, value in vis.filter.items()])
    ...
    if vis.pre_vis and vis.x == vis.pre_vis.x and vis.y == vis.pre_vis.y:
        # if len(vis.par_vis.filter) > 0:
        #    overall_points = getOverallData(vis, keys, vis.pre_vis.x, vis.pre_vis.y)
        # else:
        #    overall_points = vis.pre_vis.subgroup
        data["multiple_yAxes"] = True
        data['datas'].append({
            'data': list(map(lambda value: round(value, 2), [vis.pre_vis.subgroup[key] for key in keys])),
            # 'data': list(map(lambda value: round(value,2),[overall_points[key] for key in keys])),
            'label': 'Overall'
        })
...
def getOverallData(vis, keys, x_attribute, y_attribute):
    data = dataInfos[tool.curr_data]['data']
    overall_points = {}

    print(x_attribute, y_attribute)
    print(vis.par_vis, vis.par_vis.x, vis.par_vis.y,
          vis.par_vis.filter, vis.filter)
    print(vis.par_vis.getSubgroup(vis.y_aggre))

    if len(vis.par_vis.filter) > 0:
        print("=================expandType:", vis.expandType)
        curr_vis = vis.par_vis
        if vis.expandType == 2:
            curr_vis = vis.par_vis.par_vis
        for k, par_points in curr_vis.points.items():
            for pointID, value in par_points.items():
                if curr_vis.matchFilter(data[pointID], curr_vis.filter):
                    for key in keys:
                        if data[pointID][x_attribute] == key:
                            if key not in overall_points:
                                overall_points[key] = 0
                            else:
                                overall_points[key] += data[pointID][y_attribute]
...
def changeData(dataset_name, get_data=None):
    ...
    if get_data != None and len(list(get_data.keys())) > 3:
        print('save info')
        save_data = {key: value for key,
                     value in get_data.items() if key != 'dataset_name'}

        tool.wirteFile('result', save_data["store_dataset"], save_data)
    ...
    # get all column combination of the VisrootVizs = [vis for vis in enumerateVizs if len(vis.filter)==0]
    enumerateVizs = dataInfos[tool.curr_data]['enumerateVizs']
...
def get_options():
    ...
    if request.method == 'POST':
        ...
        if init:
            ...
            if init_model_option == "scratch":
                ...
                model = tool.read_data('AQ_pretrain') if tool.curr_data == "AQ"\
                    ...
                    else tool.read_data('Covid_pretrain') if tool.curr_data == "Covid"\
                    else tool.read_data('Covid19_pretrain') if tool.curr_data == "Covid19"\
                    else tool.read_data('Netflix_pretrain.pickle') if tool.curr_data == "Netflix"\
                    else None
        ...
        data = {}
        data['x_axis'] = [{'name': column}
                          for column in list(data_info['nominal'])]
        data['x_axis'].extend([{'name': column}
                              for column in list(data_info['temporal'])])
        data['y_axis'] = [{'name': column}
                          for column in list(data_info['quantitative'])]
...
def update_tr_data():
    ...
    if request.method == 'POST':
        ...
        visLabeled = [enumerateVizs[int(key)]
                      for key in get_data['label_data'].keys()]
        train_X.extend(
            list(map(lambda vis: list(chain(*list(vis.features.values()))), visLabeled)))
        train_y.append(
            list(map(lambda y: float(y), list(get_data['label_data'].values()))))
...
def get_vis_rec():
    ...
    if request.method == 'POST':
        get_data = json.loads(request.get_data())
        # vis index of chart in the tree view
        tree_vizs = [enumerateVizs[int(index)]
                     for index in get_data["chart_indices"]]
        userSelectedVis = enumerateVizs[int(get_data['chart_index'])]
        par_vizs = Utility.getAllParVis(userSelectedVis)
        userSelectedInsight = Utility.getUserSelectedInsight(
            userSelectedVis, get_data['click_item'])
        ...
        if init_model_option != "heuristic":
            ...
            if init_model_option == "transfer":
                train_X.extend(
                    [list(chain(*list(userSelectedVis.features.values())))])
                train_y = [[0.0]]
                decay_labels = [0.0]
                print('cross init feature length:', len(
                    list(chain(*list(userSelectedVis.features.values())))))

                model, init_model_option = Utility.trainRegression(
                    train_X, decay_labels, model, init_model_option)
        ...
        type1_visRec, type2_visRec = VisRecommendation.getVisRec(
            par_vizs, userSelectedInsight, enumerateVizs, model, tree_vizs)
        ...
        data['type1'] = [getChartData(vis, rank=i+1)
                         for i, vis in enumerate(type1_visRec)]
        data['type2'] = [getChartData(vis, rank=i+1)
                         for i, vis in enumerate(type2_visRec)]
...
def get_new_data():
    ...
    if request.method == 'POST':
        ...
        # new aggre
        data["y"] = vis.y + \
            '(' + aggre+')' if vis.y != vis.x else "percentage of count",
        ...
        if vis.x in tool.dataInfos[tool.curr_data]['nominal']:
            if sort == "desc":
                sorted_group = dict(
                    sorted(subgroup.items(), key=lambda x: x[1], reverse=True))
            elif sort == "asc":
                sorted_group = dict(
                    sorted(subgroup.items(), key=lambda x: x[1]))

        data["datas"].append(
            list(map(lambda value: round(value, 2), list(sorted_group.values()))))
        ...
        # overall
        if vis.pre_vis and vis.x == vis.pre_vis.x and vis.y == vis.pre_vis.y:
            pre_subgroup = vis.pre_vis.getSubgroup(aggre) if (
                vis.x != vis.y) else vis.pre_vis.subgroup
            data["datas"].append(list(map(lambda value: round(value, 2), [
                                 pre_subgroup[key] for key in labels])))
        ...
        if len(data) > 0:
            ...
            while(stacks):
                ...
                if len(data[curr['innerHTML']]) > 0:
                    ...
                    curr['children'] = [{'innerHTML': child}
                                        for child in data[curr['innerHTML']]]
...
def change_user():
    ...
    if request.method == 'POST':
        ...
        save_data = {key: value for key,
                     value in get_data.items() if key != 'dataset'}
        tool.wirteFile('result', save_data["store_dataset"], save_data)
        tool.save_data(model, save_data["store_dataset"])
        ...
        for dataset, dataInfo in dataInfos.items():
            ...
            while len(stack) != 0:
                ...
                for click_item in list(userSelectedVis.subgroup.keys()):
                    ...
                    type1_visRec, type2_visRec = VisRecommendation.getVisRec(
                        par_vizs, userSelectedInsight, dataInfo['enumerateVizs'])
...
def get_taiwan_map():
    ...
    if request.method == 'POST':
        ...
        taiwan_city = tool.getTaiwanCity()

    data = [taiwan_map, taiwan_city]
    rst = jsonify(data)
    rst.headers.add('Access-Control-Allow-Origin', '*')
...
@app.route('/get_world_map', methods=['GET', 'POST'])
def get_world_map():
    print("---server/get_world_map")
    if request.method == 'POST':
        get_data = json.loads(request.get_data())
        world_map = tool.getWorldMap(get_data)
        world_country = tool.getWorldCountry(get_data)
    data = [world_map, world_country]
    rst = jsonify(data)
    rst.headers.add('Access-Control-Allow-Origin', '*')
...
@app.route('/save_svgfeatures', methods=['GET', 'POST'])
def save_svgfeatures():
...
@app.route('/save_collage_logs', methods=['GET', 'POST'])
def save_collage_logs():
    print("---server/save_collage_logs")
    if request.method == 'POST':
        get_data = json.loads(request.get_data())
        save_data = {key: value for key, value in get_data.items()}

        tool.wirteFile(
            'collage_logs', save_data["curr_dataset"]+'_collage', save_data)
